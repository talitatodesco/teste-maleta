...
String Ch[10];
uint8_t horarioInicial[10];
String Cm[10];
uint8_t minutoInicial[10];

String hora[10];
uint8_t intervaloHoras[10];

String minuto[10];
uint8_t intervaloMinutos[10];
...
uint8_t horarioInicialAnterior[10];
uint8_t minutoInicialAnterior[10];

uint8_t intervaloHorasAnterior[10];
uint8_t intervaloMinutosAnterior[10];
...
void setup() {
  ...
  for (uint8_t x = 0; x < 10; x++) {
    horarioInicialAnterior[x] = 255;
    minutoInicialAnterior[x] = 255;
    intervaloMinutosAnterior[x] = 255;
    intervaloHorasAnterior[x] = 255;
...
void calculaHora(uint8_t horarioInicial, uint8_t minutoInicial, uint8_t intervaloHoras, uint8_t intervaloMinutos, uint8_t* valorHora, uint8_t* valorMinuto) {
  *valorHora = horarioInicial + intervaloHoras;
  *valorMinuto = minutoInicial + intervaloMinutos;
  ...
  while (*valorHora <= hrs || *valorMinuto <= mins) {
    incrementoDaHora(intervaloHoras, intervaloMinutos, &(*valorHora), &(*valorMinuto));
  }
}
...
void incrementoDaHora(uint8_t incHora, uint8_t incMinutos, uint8_t* intervaloHoras, uint8_t* intervaloMinutos) {
  // Correção das horas e minutos
  *intervaloMinutos += incMinutos;
  *intervaloHoras += incHora;


  if (*intervaloMinutos >= 60) {
    *intervaloHoras += *intervaloMinutos / 60;  // Soma o resultado da divisão ao valor da Hora
    *intervaloMinutos %= 60;                    // Recebe o resto da divisão, desta forma teremos o valor em minutos.
  }
...
bool houveAtualizacao(uint8_t compartimento) {
  ...
  Serial.println(horarioInicial[compartimento]);
  Serial.println(horarioInicialAnterior[compartimento]);
  Serial.println(minutoInicial[compartimento]);
  Serial.println(minutoInicialAnterior[compartimento]);
  Serial.println(intervaloHoras[compartimento]);
  Serial.println(intervaloHorasAnterior[compartimento]);
  ...
  if (horarioInicial[compartimento] != horarioInicialAnterior[compartimento]) {
    horarioInicialAnterior[compartimento] = horarioInicial[compartimento];
    status = true;
  }
  ...
  if (intervaloHoras[compartimento] != intervaloHorasAnterior[compartimento]) {
    intervaloHorasAnterior[compartimento] = intervaloHoras[compartimento];
    status = true;
  }
  if (intervaloMinutos[compartimento] != intervaloMinutosAnterior[compartimento]) {
    intervaloMinutosAnterior[compartimento] = intervaloMinutos[compartimento];
    status = true;
  }
...
void loop() {
  ...
  if (funcaoalarme) {
    ...
    if (hrs >= horamedi[0] && mins >= minmedi[0]) {
      ...
      incrementoDaHora(intervaloHoras[0], intervaloMinutos[0], &horamedi[0], &minmedi[0]);
      Serial.println("********1");
      Serial.println("Hora de tomar remédio:");
      Serial.println(intervaloHoras[0]);
      Serial.println(intervaloMinutos[0]);
      Serial.println(horamedi[0]);
      Serial.println(minmedi[0]);
    } else {
    ...
    //          CtrBuzzer(true); // Liga o Buzzer
    //          // Determina o próximo horário para tomar o remédio
    //          incrementoDaHora(hora[5], minuto[5], &horamedi[5], &minmedi[5]);
    //          Serial.println("********6");
    //          Serial.println("Hora de tomar remédio:");
    ...
    //          CtrBuzzer(true); // Liga o Buzzer
    //          // Determina o próximo horário para tomar o remédio
    //          incrementoDaHora(hora[6], minuto[6], &horamedi[6], &minmedi[6]);
    //          Serial.println("********7");
    //          Serial.println("Hora de tomar remédio:");
    ...
    //          CtrBuzzer(true); // Liga o Buzzer
    //          // Determina o próximo horário para tomar o remédio
    //          incrementoDaHora(hora[7], minuto[7], &horamedi[7], &minmedi[7]);
    //          Serial.println("********8");
    //          Serial.println("Hora de tomar remédio:");

...
void calculandoHora(/*int Ch, int Cm, int hora, int minuto,*/ int valorhora[8], int valorminuto[8], int i) {

  for (i = 0; i < 8; i++) {
    valorhora[i] = Ch[i] + hora[i];
    valorminuto[i] = Cm[i] + minuto[i];

    if (valorminuto[i] >= 60) {           //Se valor minimo for maior igual a 60
      valorhora[i] += valorminuto[i] / 60;  //Valor hora devera ser maior igual a valor minuto e dividir por 60
      valorminuto[i] %= 60;               //o resto da divisão em minutos
    }
    if (valorhora[i] >= 24) {
      valorhora[i] %= 24;
    }
...
// void incrementacaohora(uint8_t horainc, uint8_t mininc, uint8_t hora, uint8_t minuto) {
//   minuto += mininc;
//   hora += horainc;
...
// bool houveAtualizacao(uint8_t Ch[8]) {
//   previousLEDStatus = false;

//   for (int i = 0; i < 10; i++) {
//     if (Ch[i] != Chantes[i]) {

//       Chantes[i] = Ch[i];
//       previousLEDStatus = true;
//     }
//     if (Cm[i] != Cmantes[i]) {
//       Cmantes[i] = Cm[i];
//       previousLEDStatus = true;
//     }
//     if (hora[i] != horaAntes[i]) {
//       horaAntes[i] = hora[i];
//       previousLEDStatus = true;
//     }
//     if (minuto[i] != minAntes[i]) {
//       minAntes[i] = minuto[i];
//       previousLEDStatus = true;
//     }
//   }
...
//     previousLEDStatus = false;
//   }
//   return previousLEDStatus;
// }

// Função declaração alarme
void AlarmTools() {

  calculandoHora(valorhora[i], valorminuto[i]);
  ...
  switch (step) {
    ...
    case 1:
      tempoAtualCase0 = millis();
      compartimento[0] = Firebase.getInt("maleta/C1");
    ...
    case 2:
      tempoAtualCase0 = millis();
      compartimento[1] = Firebase.getInt("maleta/C2");
    ...
    case 3:
      tempoAtualCase0 = millis();
      compartimento[2] = Firebase.getInt("maleta/C3");
    ...
    case 4:
      tempoAtualCase0 = millis();
      compartimento[3] = Firebase.getInt("maleta/C4");
    ...
    case 5:
      tempoAtualCase0 = millis();
      compartimento[4] = Firebase.getInt("maleta/C5");
    ...
    case 6:
      tempoAtualCase0 = millis();
      compartimento[5] = Firebase.getInt("maleta/C6");
    ...
    case 7:
      tempoAtualCase0 = millis();
      compartimento[6] = Firebase.getInt("maleta/C7");
    ...
    case 8:
      tempoAtualCase0 = millis();
      compartimento[7] = Firebase.getInt("maleta/C8");
    ...
    case 9:
      tempoAtualCase0 = millis();
      compartimento[8] = Firebase.getInt("maleta/C9");
...
void Repeats() {
  Serial.println("Alarme disparando!!");

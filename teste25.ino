...
static uint8_t ChAnterior[8];
static uint8_t CmAnterior[8];
static uint8_t horaAnterior[8];
static uint8_t minutoAnterior[8];
...
void calculaHora(uint8_t Ch, uint8_t Cm, uint8_t hora, uint8_t minuto, uint8_t* valorHora, uint8_t* valorMinuto) {
  ...
  *valorMinuto = Cm + minuto;
  ...
  if (*valorMinuto >= 60) {
    *valorHora += *valorMinuto / 60;  // divisão  minuto com 60 e soma do resultado
    *valorMinuto %= 60;               //atribuir o resto
  }
  if (*valorHora >= 24) {
    *valorHora %= 24;  //conversão de hora
  }
}
...
void incrementoDaHora(uint8_t incHora, uint8_t incMinutos, uint8_t* hora, uint8_t* minuto) {
  ...
  *hora += incHora;
  ...
  if (*hora >= 24) {
    *hora %= 24;  // Conversão hora
  }
}


// bool houveatualizacao(uint8_t Ch[8], bool status) {
//   status = false;

//   if ( Ch[Ch[0]] != ChAnterior[Ch[0]]) {
//     ChAnterior[Ch[0]] = Ch[Ch[0]];
//     status = true;
//   }
//     if (Cm[Cm[0]] !=CmAnterior[Cm[0]]) {
//     CmAnterior[Cm[0]] = Cm[Cm[0]];
//     status = true;
//   }
//    if (hora[Ch[0]] != horaAnterior[Ch[0]]) {
//     horaAnterior[Ch[0]] = Ch[Ch[0]];
//     status = true;
//    }
//      if (minuto[Cm[0]] != minutoAnterior[Cm[0]]) {
//     minutoAnterior[Cm[0]] = minuto[Cm[0]];
//     status = true;
//   }
//   return status;
// }
  ...
  switch (step) {
    ...
    case 1:
      ...
      if (houveatualizacao(0)) {
        Serial.println("Entrei houveatualização");
...
void loop() {
  ...
  if (funcaoalarme) {
    ...
    if (hrs >= horamedi[0] && mins >= minmedi[0]) {
      Serial.println("compartimento 1");

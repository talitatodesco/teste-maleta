...
#define FIREBASE_HOST "https://malitapp-default-rtdb.firebaseio.com"
#define FIREBASE_AUTH "WcSeFf1vfUi72og13zt2ieOn14RZ4IrGIGjtvMqP"
...
bool primeiraRequisicao[10] = { true, true, true, true, true };
...
void calculaHora(uint8_t horaInicial, uint8_t minutoInicial, uint8_t intervaloHoras, uint8_t intervaloMinuto, uint8_t* valorHora, uint8_t* valorMinuto) {
  *valorHora = horaInicial + intervaloHoras;
  *valorMinuto = minutoInicial + intervaloMinuto;
...
void loop() {
    ...
    if (!funcaoalarme) {
      switch (stepTesteMaleta) {
        case 0:
          ...
          funcaoalarme = Firebase.getBool("maleta/funcaoalarme");
          todosOsAlarmes = Firebase.getBool("maleta/todososalarmes");
          stepTesteMaleta++;
  ...
  if (funcaoalarme) {
    ...
    if (hrs >= horamedi[0] && mins >= minmedi[0]) {
      ...
      Serial.println(primeiraRequisicao[0]);
      primeiraRequisicao[0] = false;
    }

    if (hrs >= horamedi[1] && mins >= minmedi[1]) {

      if (primeiraRequisicao[1] == false) {
        digitalWrite(led2, HIGH);
        delay(900);
        CtrBuzzer(true);
      }
      ...
      Serial.println(horamedi[1]);
      Serial.println(minmedi[1]);
      primeiraRequisicao[1] = false;
    }

    if (hrs >= horamedi[2] && mins >= minmedi[2]) {
      if (primeiraRequisicao[2] == false) {
        digitalWrite(led3, HIGH);
        delay(900);
        CtrBuzzer(true);
      }
      ...
      primeiraRequisicao[2] = false;
    }

    if (hrs >= horamedi[3] && mins >= minmedi[3]) {
      if (primeiraRequisicao[3] == false) {
        digitalWrite(led4, HIGH);
        delay(900);
        CtrBuzzer(true);
      }
      ...
      primeiraRequisicao[3] = false;
    }

    if (hrs >= horamedi[4] && mins >= minmedi[4]) {
      if (primeiraRequisicao[4] == false) {
        digitalWrite(led5, HIGH);
        delay(900);
        CtrBuzzer(true);
      }
  ...
  if (!funcaoalarme) {
    ...
    if (todosOsAlarmes) {
      acenderLeds();
      delay(5000);
      todosOsAlarmes = false;
      Firebase.setBool("maleta/todososalarmes", todosOsAlarmes);
    }
  }
  printLocalTime();
}

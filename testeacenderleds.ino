...
void acenderLeds(bool todos) {
  bool todosLedsStatus = Firebase.getBool("maleta/todosleds");

  if (todosLedsStatus && todos) {
  for (int i = 1; i <= 9; i++) {
    String led_statusVar = "led_statusC" + String(i);
    int led_status = Firebase.getInt("maleta/C" + String(i) + "/led_status");

    if (led_status == 1) {
      // Definir os pinos corretos para cada LED
      int pinos[] = { 15, 2, 4, 16, 17, 5, 18, 19, 21 };
      digitalWrite(pinos[i - 1], HIGH);
    }
  }
  ...
  for (int i = 1; i <= 9; i++) {
    // Definir os pinos corretos para cada LED
    int pinos[] = { 15, 2, 4, 16, 17, 5, 18, 19, 21 };
    digitalWrite(pinos[i - 1], LOW);

    // Atualizar o status dos LEDs no Firebase para desligados
    String led_statusVar = "led_statusC" + String(i);
    Firebase.setBool("maleta/C" + String(i) + "/led_status", false);
  }
}
}
...
void loop() {
  ...
  if (funcaoalarme) {
    ...
    Serial.println(hrs);
    Serial.println(mins);
    // Serial.println(horaInicial[0]);
    // Serial.println(minutoInicial[0]);
    // Serial.println(intervaloHora[0]);
    // Serial.println(intervaloMinuto[0]);

...
uint8_t ChAnterior[8];
uint8_t CmAnterior[8];
uint8_t horaAnterior[8];
uint8_t minutoAnterior[8];
...
void incrementoDaHora(uint8_t incHora, uint8_t incMinutos, uint8_t* hora, uint8_t* minuto) {
  *minuto += incMinutos;
  *hora += incHora;

  if (*minuto >= 60) {
    *hora += *minuto / 60;  // Adiciona o resultado da divisão ao valor da Hora
    *minuto %= 60;          // Recebe o resto da divisão, resultando no valor correto em minutos
  }

  if (*hora >= 24) {
    *hora %= 24;  // Correção do valor das Horas
  }
}
...
//   *hora += incHora;
//   if (*minuto >= 60) {
//     *hora += *minuto / 60;  // Soma o resultado da divisão ao valor da Hora
//     *minuto %= 60;          // Recebe o resto da divisão, desta forma teremos o valor em minutos.
//   }
...
// bool houveatualizacao(uint8_t compartimento, bool status) {
//   status = false;

//   if ( Ch[Ch[8]] != ChAnterior[Ch[8]]) {
//     ChAnterior[Ch[8]] = Ch[Ch[8]];
//     status = true;
//   }
//     if (Cm[Cm[8]] !=CmAnterior[Cm[8]]) {
//     CmAnterior[Cm[8]] = Cm[Cm[8]];
//     status = true;
//   }
//    if (hora[Ch[8]] != horaAnterior[Ch[8]]) {
//     horaAnterior[Ch[8]] = Ch[Ch[8]];
//     status = true;
//    }
//      if (minuto[Cm[8]] != minutoAnterior[Cm[8]]) {
//     minutoAnterior[Cm[8]] = minuto[Cm[8]];
//     status = true;
//   }
//   return status;
// }
...
bool houveatualizacao(uint8_t compartimento, bool status) {
  ...
  if (hora[compartimento] != horaAnterior[compartimento]) {
    horaAnterior[compartimento] = Ch[compartimento];
    status = true;
  }
...
void loop() {
  ...
  if (funcaoalarme) {
    ...
    // incrementoDaHora(&horamedi[8], &minmedi[8],hora[8], minuto[8]);  // Próxima hora de tomar o remédio
    ...
    if (hrs >= horamedi[8] && mins >= minmedi[8]) {
      ...
      // incrementoDaHora(&horamedi[8], &minmedi[8], hora[8], minuto[8]);
      Serial.println(minmedi[8]);
